...
func main() {
	...
	for scanner.Scan() {
		...
		password := []rune(line[3])
		count := strings.Count(line[3], line[2])
		...
		if (string(password[min-1]) == line[2] || string(password[max-1]) == line[2]) && !(string(password[min-1]) == line[2] && string(password[max-1]) == line[2]) {
			partTwoCount += 1
		}
	}
